...
from bot.database.queries import log_command, upsert_server_config
...
class RateLimitCommands(commands.GroupCog, name="rate_limits"):
    ...
    async def rl_set(
        ...
        # Discord passes numeric params as float; cast to int for validation and storage.
        max_open_i = int(max_open)
        cooldown_hours_i = int(cooldown_hours)
        reminder_mins_i = int(reminder_mins)

        if not (1 <= max_open_i <= 500):
            await interaction.followup.send(
        ...
        await upsert_server_config(
            ...
            max_open_deals=max_open_i,
            max_daily_volume=max_volume,
            dispute_cooldown_hours=cooldown_hours_i,
            reminder_freq_minutes=reminder_mins_i,
        ...
        await log_command(
            db, interaction.guild_id, interaction.user.id, "rate_limits set", True
        )
