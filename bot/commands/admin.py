...
from bot.database.queries import (
    get_command_stats, get_user_by_discord_id, log_command
)
...
class AdminCommands(commands.GroupCog, name="admin"):
    ...
    async def admin_backup(self, interaction: discord.Interaction):
        ...
        await log_command(db, interaction.guild_id, interaction.user.id, "admin backup", path is not None)
    ...
    async def admin_stats(self, interaction: discord.Interaction, days: int = 30):
        ...
        lines = [f"`{r['command']:<20}` calls: **{r['total']}** | ✅ {r['ok']} | ❌ {r['errors']}" for r in rows]
        embed = info_embed(
            f"📊 Command Stats — last {days} days",
            "\n".join(lines),
        )
        await interaction.followup.send(embed=embed, ephemeral=True)
        await log_command(db, interaction.guild_id, interaction.user.id, "admin stats", True)
